...
def content_factory(tmp_path_factory, deb_generic_content_factory):
    def _content_factory(name):
        file = tmp_path_factory.mktemp("content") / name
        file.write_text(str(uuid.uuid4()))
        return deb_generic_content_factory(relative_path=name, file=str(file))
...
def setup(
    ...
    distribution = deb_publication_factory(repository=repo.pulp_href, checkpoint=True)
